...
def loss_of_one_batch_with_features(batch, model, criterion, device, symmetrize_batch=False, use_amp=False, ret=None):
    view1, view2 = batch
    ignore_keys = set(['depthmap', 'dataset', 'label', 'instance', 'idx', 'true_shape', 'rng'])
    for view in batch:
        for name in view.keys():  # pseudo_focal
    ...
    with torch.cuda.amp.autocast(enabled=bool(use_amp)):
        pred1, pred2,feat_1,feat_2 = model.forward_pass_with_features(view1, view2)

        # loss is supposed to be symmetric
        with torch.cuda.amp.autocast(enabled=False):
            loss = criterion(view1, view2, pred1, pred2) if criterion is not None else None

    result = dict(view1=view1, view2=view2, pred1=pred1, pred2=pred2,feat_1=feat_1,feat_2=feat_2 ,loss=loss)
    return result[ret] if ret else result
...
@torch.no_grad()
def inference_teacher(pairs, model, device, batch_size=8, verbose=True):
    if verbose:
        print(f'>> Inference with model on {len(pairs)} image pairs')
    result = []
    ...
    for i in tqdm.trange(0, len(pairs), batch_size, disable=not verbose):
        res = loss_of_one_batch_with_features(collate_with_cat(pairs[i:i + batch_size]), model, None, device)
        result.append(to_cpu(res))
